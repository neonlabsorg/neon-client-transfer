...
describe('NEON token transfer tests', () => {
  it('Should return ethers contracts if nothing was passed in getContracts', async () => {
    const web3 = new Web3(new Web3.providers.HttpProvider(NEON_PROXY_URL!));
    const ethersContracts = getContracts();
    expect(ethersContracts).toBeDefined();
    expect(ethersContracts.erc20ForSPLContract).toBeInstanceOf(Interface);
    expect(ethersContracts.neonWrapperContract).toBeInstanceOf(Interface);
    expect(ethersContracts.neonWrapper2Contract).toBeInstanceOf(Function);
    const web3Contracts = getContracts(web3);
    expect(web3Contracts).toBeDefined();
    expect(web3Contracts.erc20ForSPLContract).not.toBeInstanceOf(Interface);
  });
  ...
  it(`Should wrap SOL -> wSOL and transfer 0.1 wSOL from Solana to Neon`, async () => {
    ...
    if (id > -1) {
      ...
      try {
        const transaction = await createWrapAndTransferSOLTransactionWeb3<JsonRpcProvider>(connection, provider, neonProxyRpcApi, neonProxyStatus, neonEvmProgram, solanaWallet.publicKey, neonWallet.address, wSOL, amount, CHAIN_ID);
