...
describe('Transfer tests', () => {
  ...
  it(`Should transfer 0.1 NEON from Solana to Neon`, async () => {
    ...
    try {
      ...
      const balanceMint = await neonBalance(web3, account.address);
  ...
  it(`Should transfer 0.1 NEON from Neon to Solana`, async () => {
    ...
    try {
      ...
      const hash = await sendNeonTransaction(web3, transaction, account);
  ...
  it('Should wrap 1 NEON to wNEON in Neon network', async () => {
    ...
    if (id > -1) {
      ...
      try {
        ...
        const wrapHash = await sendNeonTransaction(web3, wrapTransaction, account);
  ...
  it('Should withdraw 0.1 wNEON from Neon to Solana', async () => {
    ...
    if (id > -1) {
      ...
      try {
        ...
        const unwrapHash = await sendNeonTransaction(web3, unwrapTransaction, account);
  ...
  it.skip('Should wrap and unwrap 0.1 SOL <-> wSOL', async () => {
    ...
    if (id > -1) {
      ...
      try {
        ...
        const signature = await sendSolanaTransaction(connection, unwrapTransaction, [signer], true, { skipPreflight: true });
        expect(signature.length).toBeGreaterThan(0);
  ...
  it(`Should transfer 0.1 wSOL from Neon to Solana and unwrap wSOL -> SOL`, async () => {
    ...
    if (id > -1) {
      ...
      try {
        ...
        await delay(1e3);
        const signedNeonTransaction = await sendNeonTransaction(web3, neonTransaction, account);
