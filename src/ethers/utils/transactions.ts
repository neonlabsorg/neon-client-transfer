...
export function useContractMethods<T = any>(i: T): ContractMethods<string | Promise<TransactionResponse>> {
  const contracts = getContracts(i);

  if (i && i instanceof Web3) {
    return {
      claimTransactionData: (associatedToken: PublicKey, neonWallet: string, amount: Amount): string => {
        const claimTo = (contracts.erc20ForSPLContract as Web3Contract<ContractAbi>).methods.claimTo(associatedToken.toBuffer(), neonWallet, amount);
        return claimTo.encodeABI();
      },
      mintNeonTransactionData: (associatedToken: PublicKey, splToken: SPLToken, amount: Amount): string => {
        const fullAmount = toFullAmount(amount, splToken.decimals);
        return (contracts.erc20ForSPLContract as Web3Contract<ContractAbi>).methods.transferSolana(associatedToken.toBuffer(), fullAmount).encodeABI();
      },
      neonTransactionData: (solanaWallet: PublicKey): string => {
        return (contracts.neonWrapperContract as Web3Contract<ContractAbi>).methods.withdraw(solanaWallet.toBuffer()).encodeABI();
      },
      wrappedNeonTransactionData: (token: SPLToken, amount: Amount): string => {
        const value = toWei(amount.toString(), 'ether');
        return (contracts.neonWrapper2Contract(token.address) as Web3Contract<ContractAbi>).methods.withdraw(value).encodeABI();
      }
    };
  }
