    ...
    actions: {
        ...
        setTokenList() {
            ...
            if (web3Store.chainId === networkUrls[0].id) {
                this.tokenList.unshift({ ...NEON_TOKEN_MODEL, address_spl: transactionStore.networkTokenMint.toBase58() });
            } else {
                const wSOL = this.tokenList.find(i => i.symbol === 'wSOL');
                this.tokenList.unshift({ ...wSOL, ...SOL_TOKEN_MODEL, address_spl: transactionStore.networkTokenMint.toBase58() });
            }
        },
