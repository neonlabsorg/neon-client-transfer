...
export async function neonTransferMintTransaction(connection, proxyStatus, neonEvmProgram, solanaWallet, neonWallet, emulateSigner, neonKeys, neonTransaction, splToken, amount, chainId) {
...
export function createAccountBalanceInstruction(solanaWallet, neonPDAWallet, neonEvmProgram, neonWallet, chainId) {
    ...
    const a = Buffer.from([45 /* EvmInstruction.AccountCreateBalance */]);
    const b = Buffer.from(neonWallet.slice(2), 'hex');
...
export async function createClaimInstruction(proxyApi, signedTransaction) {
    try {
        let neonEmulate;
