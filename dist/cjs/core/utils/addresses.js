...
function neonWalletProgramAddress(etherKey, neonEvmProgram) {
    const keyBuffer = Buffer.from((0, utils_1.isValidHex)(etherKey) ? etherKey.replace(/^0x/i, '') : etherKey, 'hex');
...
function authAccountAddress(neonWallet, neonEvmProgram, splToken) {
    const neonAccountAddressBytes = Buffer.concat([Buffer.alloc(12), Buffer.from((0, utils_1.isValidHex)(neonWallet) ? neonWallet.replace(/^0x/i, '') : neonWallet, 'hex')]);
    const neonContractAddressBytes = Buffer.from((0, utils_1.isValidHex)(splToken.address) ? splToken.address.replace(/^0x/i, '') : splToken.address, 'hex');
    const seed = [
        new Uint8Array([3 /* AccountHex.SeedVersion */]),
...
function collateralPoolAddress(neonWalletPDA, collateralPoolIndex) {
    ...
    const b = Buffer.from((0, utils_1.toBytesInt32)(collateralPoolIndex));
